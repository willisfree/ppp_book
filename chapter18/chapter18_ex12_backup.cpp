...
bool make_shoot(Room* map, int& arrows)		//ask player number of rooms and change number of arrows; return true if player lose or win
{
	if (map == nullptr)
	...
	cin >> count;
	if (count > arrows)
			cout << "You haven't enought arrows\n";
	while (count < 1 || count > arrows) {
	...
	while (true) {
		...
		while (!end_of_game) {
			...
			if (ans == move) {
				...
				end_of_game = make_shoot(map, arrows);
